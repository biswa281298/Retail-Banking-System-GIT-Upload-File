...
@RestController
public class CustomerController {  // /customer end point
	...
	public ResponseEntity<?> createCustomer(@RequestHeader("Authorization") String token,@Valid @RequestBody CustomerEntity customer,BindingResult bindingResult) throws DateTimeException, BindException{
		...
		{
			throw new ResponseStatusException(HttpStatus.NOT_ACCEPTABLE,bindingResult.getFieldErrors().stream().map(f->f.getField()+" -> "+f.getDefaultMessage()).collect(Collectors.toList()).toString());
		}
