...
public class AccountController {
	...
	public ResponseEntity<?> createAccount(@RequestHeader("Authorization") String token,@PathVariable String customerId,@Valid @RequestBody Account account,BindingResult result) {
		accountServiceImpl.hasEmployeePermission(token);
		if(result.hasErrors()) {
			throw new ResponseStatusException(HttpStatus.NOT_ACCEPTABLE,result.getFieldErrors().stream().map(f->f.getField()+" -> "+f.getDefaultMessage()).collect(Collectors.toList()).toString());	
		}else {
